...
4. Decides completion from the conversation data, not the page: it reads the conversation every 2 seconds through the same detail operation the history agent uses, finds this request's own user message, and waits for the answer after it (ChatGPT: a finished assistant message; claude.ai: a `stop_reason`, or the same text on 4 reads spanning at least 10 seconds). The wait is bounded by the 8 minute request timeout.
