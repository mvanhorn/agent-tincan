...
Attaching local files works only in the MCP server running on the agent's own machine (`tincan mcp`). ChatGPT through the gateway sees images it receives but cannot attach files.
